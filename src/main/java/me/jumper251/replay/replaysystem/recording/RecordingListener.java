...
public class RecordingListener extends AbstractListener {
	...
	public void onInteract (PlayerInteractEvent e) {
		...
		if (recorder.getPlayers ().contains (p.getName ())) {
			...
			if (e.getAction () == Action.LEFT_CLICK_BLOCK && p.getTargetBlock ((Set<Material>)null, 5).getType () == Material.FIRE) {
				...
				ItemData before = new ItemData (Material.FIRE, 0);
				ItemData after  = new ItemData (Material.AIR, 0);
	...
	public void onThrow (PlayerDropItemEvent e) {
		...
		if (this.recorder.getPlayers ().contains (p.getName ())) {
			InvData data = NPCManager.copyFromPlayer (p, true, true);
			if (data.getMainHand () != null && p.getItemInHand () != null && p.getItemInHand ().getAmount () <= 1 && p.getItemInHand ().getType () == data.getMainHand ().getId ()) {
				data.setMainHand (null);
			}
	...
	public void onPlace (BlockPlaceEvent e) {
		...
		if (this.recorder.getPlayers ().contains (p.getName ())) {
			...
			ItemData before = new ItemData (e.getBlockReplacedState ().getType (), e.getBlockReplacedState ().getData ().getData ());
			ItemData after  = new ItemData (e.getBlockPlaced ().getType (), e.getBlockPlaced ().getData ());
	...
	public void onBreak (BlockBreakEvent e) {
		...
		if (this.recorder.getPlayers ().contains (p.getName ())) {
			...
			ItemData before = new ItemData (e.getBlock ().getType (), e.getBlock ().getData ());
			ItemData after  = new ItemData (Material.AIR, 0);
	...
	public void onFill (PlayerBucketFillEvent e) {
		...
		if (this.recorder.getPlayers ().contains (p.getName ())) {
			...
			ItemData before = new ItemData (e.getBlockClicked ().getState ().getType (), e.getBlockClicked ().getState ().getData ().getData ());
			ItemData after  = new ItemData (Material.AIR, 0);
	...
	public void onEmpty (PlayerBucketEmptyEvent e) {
		...
		if (this.recorder.getPlayers ().contains (p.getName ())) {
			...
			ItemData before = new ItemData (block.getType (), block.getData ());
			ItemData after  = new ItemData (e.getBucket () == Material.WATER_BUCKET ? Material.STATIONARY_WATER : Material.STATIONARY_LAVA, 0);
