...
public class ReplayingUtils {
	...
	public void handleAction (ActionData action, ReplayData data, boolean reversed) {
		...
		if (action.getType () == ActionType.PACKET && this.replayer.getNPCList ().containsKey (action.getName ())) {
			...
			if (action.getPacketData () instanceof VelocityData) {
				...
				int entID             = -1;
				if (hooks.containsKey (velocity.getId ()))
					entID = hooks.get (velocity.getId ());
	...
	private void setBlockChange (BlockChangeData blockChange) {
		...
		new BukkitRunnable () {
			...
			@Override
			public void run () {
				if (blockChange.getAfter ().getId () == Material.AIR && blockChange.getBefore ().getId () != Material.AIR && blockChange.getBefore ().getId () != Material.FIRE && blockChange.getBefore ().getId () != Material.STATIONARY_LAVA && blockChange.getBefore ().getId () != Material.STATIONARY_WATER) {
				...
				if (id == Material.STATIONARY_WATER)
					id = Material.WATER;
				if (id == Material.STATIONARY_LAVA)
					id = Material.LAVA;
				...
				if (ConfigManager.REAL_CHANGES) {
					if (VersionUtil.isCompatible (VersionEnum.V1_13) || VersionUtil.isCompatible (VersionEnum.V1_14) || VersionUtil.isCompatible (VersionEnum.V1_15) || VersionUtil.isCompatible (VersionEnum.V1_16)) {
						...
						loc.getBlock ().setTypeIdAndData (id.getId (), (byte)subId, true);
					}
				} else {
					if (VersionUtil.isCompatible (VersionEnum.V1_13) || VersionUtil.isCompatible (VersionEnum.V1_14) || VersionUtil.isCompatible (VersionEnum.V1_15) || VersionUtil.isCompatible (VersionEnum.V1_16)) {
						replayer.getWatchingPlayer ().sendBlockChange (loc, id, (byte)subId);
					} else {
						replayer.getWatchingPlayer ().sendBlockChange (loc, id.getId (), (byte)subId);
					}
				}
			}
		}.runTask (ReplaySystem.getInstance ());
