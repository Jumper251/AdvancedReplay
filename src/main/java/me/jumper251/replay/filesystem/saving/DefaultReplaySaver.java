...
public class DefaultReplaySaver implements IReplaySaver {
	...
	public void reformatAll () {
		...
		if (DIR.exists ()) {
			Arrays.asList (DIR.listFiles ()).stream ().filter (file -> (file.isFile () && file.getName ().endsWith (".replay"))).map (File::getName).collect (Collectors.toList ()).forEach (file -> reformat (file.replaceAll ("\\.replay", "")));
		}
