...
public class WrapperPlayServerSpawnEntity extends AbstractPacket {
    ...
    public Vector getVelocity() {
        if (VersionUtil.isAbove(VersionEnum.V1_21_10)) {
            return getHandle().getVectors().read(0);
        } else {
            return new Vector(getOptionalSpeedX(), getOptionalSpeedY(), getOptionalSpeedZ());
        }
    }

    public void setVelocity(Vector velocity) {
        if (VersionUtil.isAbove(VersionEnum.V1_21_10)) {
            getHandle().getVectors().write(0, velocity);
        } else {
            setOptionalSpeedX(velocity.getX());
            setOptionalSpeedY(velocity.getY());
            setOptionalSpeedZ(velocity.getZ());
        }
    }
