...
public class PacketRecorder extends AbstractListener {
  ...
  public void register() {
    ...
    // TODO add more server packets
    this.packetAdapter = new PacketAdapter(
        ReplaySystem.getInstance(), ListenerPriority.HIGHEST,
        PacketType.Play.Client.POSITION, PacketType.Play.Client.POSITION_LOOK,
        PacketType.Play.Client.LOOK, PacketType.Play.Client.ENTITY_ACTION,
        PacketType.Play.Client.ARM_ANIMATION, PacketType.Play.Client.BLOCK_DIG,
        ...
        PacketType.Play.Server.ENTITY_LOOK, PacketType.Play.Server.POSITION,
      ...
      public void onPacketReceiving(PacketEvent event) {
        ...
        if (event.getPlayer() != null &&
            recorder.getPlayers().contains(event.getPlayer().getName())) {
          Player p = event.getPlayer();

          PacketData data = null;
          if (event.getPacketType() == PacketType.Play.Client.POSITION) {
            WrapperPlayClientPosition packet =
                new WrapperPlayClientPosition(event.getPacket());
            data = new MovingData(packet.getX(), packet.getY(), packet.getZ(),
                                  p.getLocation().getPitch(),
                                  p.getLocation().getYaw());

            if (recorder.getData().getWatcher(p.getName()).isBurning() &&
                p.getFireTicks() <= 20) {
              ...
              addData(
                  ...
                  new MetadataUpdate(
                      false,
                      recorder.getData().getWatcher(p.getName()).isBlocking()));
            }
          }

          if (event.getPacketType() == PacketType.Play.Client.LOOK) {
            WrapperPlayClientLook packet =
                new WrapperPlayClientLook(event.getPacket());
            data = new MovingData(
                p.getLocation().getX(), p.getLocation().getY(),
                p.getLocation().getZ(), packet.getPitch(), packet.getYaw());
          }

          if (event.getPacketType() == PacketType.Play.Client.POSITION_LOOK) {
            WrapperPlayClientPositionLook packet =
          ...
          if (event.getPacketType() == PacketType.Play.Client.ENTITY_ACTION) {
            WrapperPlayClientEntityAction packet =
                new WrapperPlayClientEntityAction(event.getPacket());
            if (packet.getAction() == PlayerAction.START_SNEAKING ||
                packet.getAction() == PlayerAction.STOP_SNEAKING) {
          ...
          if (event.getPacketType() == PacketType.Play.Client.BLOCK_DIG) {
            WrapperPlayClientBlockDig packet =
                new WrapperPlayClientBlockDig(event.getPacket());
            ...
            if (packet.getStatus() == PlayerDigType.RELEASE_USE_ITEM) {
              if (recorder.getData().getWatcher(p.getName()).isBlocking()) {
                ...
                addData(
                    p.getName(),
                    new MetadataUpdate(
                        recorder.getData().getWatcher(p.getName()).isBurning(),
                        false));
              }
            }
          }
      ...
      public void onPacketSending(PacketEvent event) {
        ...
        if (event.getPacketType() == PacketType.Play.Server.SPAWN_ENTITY) {
          WrapperPlayServerSpawnEntity packet =
              new WrapperPlayServerSpawnEntity(event.getPacket());

          com.comphenix.packetwrapper.old
              .WrapperPlayServerSpawnEntity oldPacket =
              new com.comphenix.packetwrapper.old.WrapperPlayServerSpawnEntity(
                  event.getPacket());
          ...
          if (VersionUtil.isCompatible(VersionEnum.V1_8)) {
            location =
                new LocationData(oldPacket.getX(), oldPacket.getY(),
                                 oldPacket.getZ(), p.getWorld().getName());
          } else {
            location = new LocationData(packet.getX(), packet.getY(),
                                        packet.getZ(), p.getWorld().getName());
          }

          if ((type == 2 || (VersionUtil.isAbove(VersionEnum.V1_14) &&
                             event.getPacket().getEntityTypeModifier().read(
                                 0) == EntityType.DROPPED_ITEM)) &&
              !spawnedItems.contains(packet.getEntityID())) {
            Entity en = packet.getEntity(p.getWorld());
            if (en != null && en instanceof Item) {
              Item item = (Item)en;
              LocationData velocity = LocationData.fromLocation(
                  item.getVelocity().toLocation(p.getWorld()));
          ...
          if ((type == 90 || (VersionUtil.isAbove(VersionEnum.V1_14) &&
                              event.getPacket().getEntityTypeModifier().read(
                                  0) == EntityType.FISHING_HOOK)) &&
              !spawnedHooks.contains(packet.getEntityID())) {
            location.setYaw(packet.getYaw());
            location.setPitch(packet.getPitch());

            if (VersionUtil.isCompatible(VersionEnum.V1_8)) {
              addData(p.getName(),
                      new FishingData(oldPacket.getEntityID(), location,
                                      oldPacket.getOptionalSpeedX(),
                                      oldPacket.getOptionalSpeedY(),
                                      oldPacket.getOptionalSpeedZ()));
            } else {
              addData(p.getName(),
                      new FishingData(packet.getEntityID(), location,
                                      packet.getOptionalSpeedX(),
                                      packet.getOptionalSpeedY(),
                                      packet.getOptionalSpeedZ()));
            }
        ...
        if (event.getPacketType() ==
                PacketType.Play.Server.SPAWN_ENTITY_LIVING &&
            ConfigManager.RECORD_ENTITIES) {
          WrapperPlayServerSpawnEntityLiving packet =
              new WrapperPlayServerSpawnEntityLiving(event.getPacket());

          EntityType type = packet.getType();
          if (type == null)
            type = packet.getEntity(p.getWorld()).getType();
          ...
          if (!spawnedEntities.containsKey(packet.getEntityID())) {
            ...
            if (VersionUtil.isCompatible(VersionEnum.V1_8)) {
              com.comphenix.packetwrapper.old
                  .WrapperPlayServerSpawnEntityLiving oldPacket =
                  new com.comphenix.packetwrapper.old
                      .WrapperPlayServerSpawnEntityLiving(event.getPacket());
              location =
                  new LocationData(oldPacket.getX(), oldPacket.getY(),
                                   oldPacket.getZ(), p.getWorld().getName());
            } else {
              location =
                  new LocationData(packet.getX(), packet.getY(), packet.getZ(),
                                   p.getWorld().getName());
            }

            EntityData entData = new EntityData(0, packet.getEntityID(),
        ...
        if (event.getPacketType() == PacketType.Play.Server.ENTITY_DESTROY) {
          WrapperPlayServerEntityDestroy packet =
              new WrapperPlayServerEntityDestroy(event.getPacket());
          ...
          for (int id : packet.getEntityIDs()) {
            ...
            if (spawnedEntities.containsKey(id) &&
                (idLookup.get(id) == null ||
                 (idLookup.get(id) != null && idLookup.get(id).isDead()))) {

              addData(p.getName(),
                      new EntityData(1, id,
                                     spawnedEntities.get(id).getLocation(),
                                     spawnedEntities.get(id).getType()));
        ...
        if (event.getPacketType() == PacketType.Play.Server.ENTITY_VELOCITY) {
          WrapperPlayServerEntityVelocity packet =
              new WrapperPlayServerEntityVelocity(event.getPacket());

          if (spawnedHooks.contains(packet.getEntityID()) ||
              (entityLookup.containsKey(packet.getEntityID()) &&
               entityLookup.get(packet.getEntityID())
                   .equalsIgnoreCase(p.getName()))) {

            addData(p.getName(), new VelocityData(packet.getEntityID(),
                                                  packet.getVelocityX(),
                                                  packet.getVelocityY(),
                                                  packet.getVelocityZ()));
          }
        ...
        if (event.getPacketType() == PacketType.Play.Server.REL_ENTITY_MOVE) {
          WrapperPlayServerRelEntityMove packet =
              new WrapperPlayServerRelEntityMove(event.getPacket());

          if (entityLookup.containsKey(packet.getEntityID()) &&
              entityLookup.get(packet.getEntityID())
                  .equalsIgnoreCase(p.getName())) {
            Location loc = packet.getEntity(p.getWorld()).getLocation();

            addData(p.getName(),
                    new EntityMovingData(packet.getEntityID(), loc.getX(),
        ...
        if (event.getPacketType() ==
            PacketType.Play.Server.REL_ENTITY_MOVE_LOOK) {
          WrapperPlayServerRelEntityMoveLook packet =
              new WrapperPlayServerRelEntityMoveLook(event.getPacket());
        ...
        if (event.getPacketType() == PacketType.Play.Server.ENTITY_TELEPORT) {
          WrapperPlayServerEntityTeleport packet =
              new WrapperPlayServerEntityTeleport(event.getPacket());
        ...
        if (event.getPacketType() == PacketType.Play.Server.BLOCK_CHANGE) {
          ...
          addData(
        	p.getName(),
        	new BlockChangeData(packet.getLocation()));
        }
      }
    };
  ...
  public void addData(String name, PacketData data) {
    if (!optimizer.shouldRecord(data))
      return;
  ...
  public HashMap<Integer, String> getEntityLookup() { return entityLookup; }
